...
/*
* using dfs to find the minimum hp to go through the dungeon
*/
void get_through_dungeon(const vector<vector<int>> dungeon, int x, int y,
	int row, int col, int& m_hp, int cur_m_hp, int cur_hp) {
	if (x >= row || y >= col) {
		return;
	}
	// find the princess
	int dungeon_hp = dungeon[x][y];
	if (x == row - 1 && y == col - 1) {
		if (dungeon_hp >= 0 && m_hp > cur_m_hp) {
			m_hp = cur_m_hp;
		}
		else if (dungeon_hp < 0) {
			if (abs(dungeon_hp) >= cur_hp) {
				cur_m_hp += abs(dungeon_hp	 + cur_hp) + 1;
			}
			if (m_hp > cur_m_hp) {
				m_hp = cur_m_hp;
			}
	...
	get_through_dungeon(dungeon, x + 1, y, row, col, m_hp, cur_m_hp, cur_hp);
	get_through_dungeon(dungeon, x, y + 1, row, col, m_hp, cur_m_hp, cur_hp);
}
...
int calculateMinimumHP(vector<vector<int>> &dungeon) {
	...
	int m_hp = INT_MAX;
	get_through_dungeon(dungeon, 0, 0, row, col, m_hp, 1, 1);

	return m_hp;
}

void test_ref_parameter(int& a) {
	int x = 24;
	cout << "before function invoke, a = " << a << endl;
	a = x;
}

void test1() {
	int a = 12;
	test_ref_parameter(a);
	cout << "after function invoke, a = " << a << endl;
}
...
void test2() {
	...
	cout << "The minimum hp to get through the dungeon is: "
		<< calculateMinimumHP(dungeon) << endl;
}

void test3() {
	// dfs is not effcient to pass the following case
